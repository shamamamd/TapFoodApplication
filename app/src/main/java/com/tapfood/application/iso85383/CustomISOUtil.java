...
public class CustomISOUtil {
    ...
    public static String formatDouble(double d, int len) {
        ...
        try {
            if (len > 3)
                prefix = CustomISOUtil.padleft(prefix,len-3,' ');
            suffix = CustomISOUtil.zeropad(suffix, 2);
    ...
    public static String protect (String s, char mask) {
        ...
        try {
            ...
            if (charCount == 2 ) {
                ...
                s = CustomISOUtil.padright(s, len, mask);
            }
        } catch (ISOException e){
    ...
    public static String formatAmountConversionRate(double convRate) throws ISOException {
        ...
        if (x > 9)
            bds = CustomISOUtil.zeropad(bds, bds.length() + x - 9);
        String ret = CustomISOUtil.zeropadRight(bds, 7);
        return Math.min(9, x) + CustomISOUtil.takeFirstN(ret, 7);
    }
